...
for dim in range(350,900,50):
	...
	#model.add(Dropout(rate=0.8,seed=random.randint(0,99)))
	model.add(Dense(32, activation='relu'))
	#model.add(Dropout(0.4,seed=random.randint(0,99)))
	...
	pca.fit(tempTraces)
	joblib.dump(pca, "./PCA_for_mlp.m")
	'''
	pca = joblib.load("./PCA_for_mlp.m")
	'''
	tempTraces = pca.transform(tempTraces)
	print(np.linalg.cond(tempTraces,p=None))

	print("完成PCA")
	atkTraces = pca.transform(atkTraces)

	history = model.fit(tempTraces, to_categorical(tempHW), epochs=10, batch_size=16,validation_split=0.05)
